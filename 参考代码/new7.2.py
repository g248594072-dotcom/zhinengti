...
from tkinter import Tk, filedialog, messagebox, Toplevel, Frame
...
try:
    # 1) 选择文件（支持拖放；未安装 tkinterdnd2 则回退为“按钮选择”）
    file_drop_window = FileDropWindow(root, "请选择 2 个 Excel 文件（可拖拽）", 2, dnd_enabled=_dnd_enabled)
    root.wait_window(file_drop_window)

    if not file_drop_window.selected:
        raise ValueError("未选择文件！")

    file1, file2 = file_drop_window.file_paths[0], file_drop_window.file_paths[1]

    # 获取保存目录（使用第一个文件的目录）
    save_dir = os.path.dirname(file1)

    # 文件名使用“当天日期”（避免你每次都要选名字/地址）
    ...
    grouped_sorted = sorted(grouped, key=lambda d: (-d["总数"], d["姓名"]))
        ...
        # ====== 一次性调试：看看会话标签原文以及解析出来的金额 ======
    if "会话标签" in merged_df.columns:
        sample = merged_df["会话标签"].dropna().astype(str).head(10).tolist()
        lines = []
        for x in sample:
            lines.append(f"{x}  ->  {calc_amount_from_tags(x)}")
        messagebox.showinfo("会话标签金额调试(前10条)", "\n".join(lines) if lines else "会话标签列前10条为空")
    else:
        messagebox.showerror("缺列", "没有找到列：会话标签")
